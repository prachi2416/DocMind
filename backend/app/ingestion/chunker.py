    ...
    def split(
        ...
        for page in pages:
            ...
            if len(page_text) <= self.chunk_size:
                chunk_meta = {**base_meta, **page.metadata}
                chunks.append(Chunk(text=page_text, metadata=chunk_meta, index=chunk_index))
                chunk_index += 1
                continue
            ...
            for text in page_chunks:
                if not text.strip():
                    continue
                chunk_meta = {**base_meta, **page.metadata}
                chunks.append(Chunk(text=text.strip(), metadata=chunk_meta, index=chunk_index))
                chunk_index += 1
